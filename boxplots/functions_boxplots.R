# These are the functions used in consistency_script.R
...
################################################################################
#MCAR X1
make_data1 <- function(size=100, noise=0.2, prob=0.2, rho=0.5, dim=10) {
...
#MAR X1
make_data2 <- function(size=100, noise=0.2, prob=0.2, rho=0.5, dim=10) {
    Sigma = rho*matrix(rep(1, dim**2), nrow = dim)+(1-rho)*diag(1, nrow=dim)
    X <- as.data.frame(mvrnorm(size, mu=rep(1, dim), Sigma=Sigma))
    y <- X[,1]**2 + rnorm(size, sd=noise)
...
#MNAR X1
make_data3 <- function(size=100, noise=0.2, prob=0.2, rho=0.5, dim=10) {
    Sigma = rho*matrix(rep(1, dim**2), nrow = dim)+(1-rho)*diag(1, nrow=dim)
    X <- as.data.frame(mvrnorm(size, mu=rep(1, dim), Sigma=Sigma))
    y <- X[,1]**2 + rnorm(size, sd=noise)
...
################################################################################
#MCAR, linear
make_data4 <- function(size=100, noise=0.2, prob=0.2, rho=0.5, dim=10) {
    Sigma = rho*matrix(rep(1, dim**2), nrow = dim)+(1-rho)*diag(1, nrow=dim)
    X <- as.data.frame(mvrnorm(size, mu=rep(1, dim), Sigma=Sigma))
    y <- as.matrix(X)%*%c(1, 2, -1, 3, -0.5, -1, 0.3, 1.7, 0.4, -0.3) + rnorm(size, sd=noise)
...
#MCAR, Friedman
make_data5 <- function(size=100, noise=0.2, prob=0.2, rho=0.5, dim=10) {
    Sigma = rho*matrix(rep(1, dim**2), nrow=dim)+(1-rho)*diag(1, nrow=dim)
    X <- as.data.frame(mvrnorm(size, mu=rep(1, dim), Sigma=Sigma))
    y <- (10*sin(pi*X[,1]*X[,2]) + 20*(X[,3]-0.5)**2 + 10*X[,4] + 5*X[,5] + noise*rnorm(size))
...
make_data6 <- function(size=100, noise=0.2, prob=0.2, rho=0.5, dim=10) {
    xnoise <- 0.05
    Xi <- 3*(runif(size)-1)
...
preprocess <- function(df, strategy, withpattern, parameters) {
    ...
    if (strategy == "mean") {
        mu <- parameters[["mean"]]
        X.imput <- sapply(1:ncol(X), function(x) ifelse(is.na(X[,x]), mu[x], X[,x]))
    ...
    if (withpattern==TRUE) {
        ...
        indicators.addfactor <- data.frame(lapply(indicators.factor, function(col) factor(col, levels=c("FALSE", "TRUE"))))
...
run_scores <- function(model, strategy, withpattern, dataset,
    ...
    # dataset: "make_model1", "make_model2", "make_model3"
    ...
    for (size in sizes) {
        ...
        for (k in 1:n_rep) {
            ...
            if (dataset=="make_data1") {
                data.raw <- make_data1(size=size, noise=noise, prob=prob, rho=rho, dim=n_features)
            } else if (dataset=="make_data2") {
                data.raw <- make_data2(size=size, noise=noise, prob=prob, rho=rho, dim=n_features)
            } else if (dataset=="make_data3") {
                data.raw <- make_data3(size=size, noise=noise, prob=prob, rho=rho, dim=n_features)
            } else if (dataset=="make_data4") {
                data.raw <- make_data4(size=size, noise=noise, prob=prob, rho=rho, dim=n_features)
            } else if (dataset=="make_data5") {
                data.raw <- make_data5(size=size, noise=noise, prob=prob, rho=rho, dim=n_features)
            } else if (dataset=="make_data6") {
                data.raw <- make_data6(size=size, noise=noise, prob=prob, rho=rho, dim=n_features)
            ...
            param <- fit.preprocess(train.raw, strategy=strategy, withpattern=withpattern)
            train <- preprocess(train.raw, strategy=strategy, withpattern=withpattern, parameters=param)
            test <- preprocess(test.raw, strategy=strategy, withpattern=withpattern, parameters=param)

            if (model=="rpart") {
                reg <- rpart(y~., data=train, control=rpart.control(minbucket=min_samples_leaf,cp=0.0, xval=1))
                res <- predict(reg, subset(test, select=-c(y)), type="vector")
            } else if (model=="ctree") {
                reg <- ctree(y~., data=train, controls=ctree_control(minbucket=min_samples_leaf, mincriterion=0.0))
