...
bayes_data1 <- function(size=megasize, noise=0.1, prob=0.4, rho=0.5, dim=10, verbose=F) {
    VARY = 25
...
bayes_data2 <- function(size=megasize, noise=0.1, prob=0.4, rho=0.5, dim=10, verbose=F) {
    VARY = 1702
    thisscore = rep(0, n_repmc)
    for (k in 1:n_repmc) {
        cat(paste0(k,"..."))
        ...
        for (t in 1:n_repmi) {
            if (verbose) cat(paste0(t,"..."))
            X_imp <- imp.norm(s, thetahat, as.matrix(X))
            res <- res + 10*sin(pi*X_imp[,1]*X_imp[,2]) + 20*(X_imp[,3]-0.5)**2 + 10*X_imp[,4] + 5*X_imp[,5]
        }
        if (verbose) cat('\n')
        res <- res / n_repmi
...
bayes_data3 <- function(size=megasize, noise=0.1, prob=0.4, rho=0.5, dim=10, verbose=F) {
    VARY = 10825
    thisscore = rep(0, n_repmc)
    for (k in 1:n_repmc) {
        cat(paste0(k,"..."))
