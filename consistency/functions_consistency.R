...
run_scores <- function(model, strategy, withpattern, dataset,
                       sizes, n_rep, prob, n_features, noise, rho, min_samples_leaf, seed, debug=FALSE) {
    ...
    if (debug) {
        ...
        write.table(result,
                    file=paste0("results/", dataset, "_", model, "_", strategy, "_", withpattern, ".csv"),
                    sep=" ", row.names=FALSE)
    }
}
